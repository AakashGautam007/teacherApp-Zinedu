...
import { AppColors, STYLES, typography } from "../../../appStyles";
import { fontFamilyConstants, fontSizeConstants } from "../../../appStyles";
...
const getKeyByValue = (value) => {
  ...
  let foundKey = Object.keys(AllPlatformFileTypes.mimeTypes).find(
    (key) =>
      AllPlatformFileTypes.mimeTypes[key] === value ||
      AllPlatformFileTypes.utis[key] === value ||
      AllPlatformFileTypes.extensions[key] === value
  );
...
export const AttachmentButton = (props) => {
  ...
  const openDocumentPicker = multiple
      ...
      ImagePicker.launchImageLibrary(
        ...
        async (response) => {
          ...
          if (response.didCancel) {
            ...
            console.log("here");
            if (await checkFiles(response?.assets)) {
              // let name
              // if (Platform.OS == "ios") {
              ...
              multiple
                ? setFileList([...fileList, ...response.assets])
                : setFileList([...fileList, response.assets]);
            }
            setLoading(false);
          }
  ...
  useEffect(() => {
    ...
    console.log("tempFileFormatArray", tempFileFormatArray);
  ...
  const isValidFile = async ({ fileSize, fileResponse }) => {
    if (fileSize > maxAllowedFileSize) {
      alert(
        `Maximum allowed file size is ${roundOff(
    ...
    if (
      allowedFileFormats &&
      !allowedFileFormats.includes(fileResponse?.type)
    ) {
  ...
  const checkMultipleFiles = async (fileResponse) => {
    let i = 0,
      fileSize,
      flag = true;
    console.log("checkMultipleFiles", fileResponse);
    do {
      ...
      if (
        !((await isValidFile({ fileSize, fileResponse: response })) && flag)
      ) {
        flag = false;
        break;
      }
  ...
  const checkFiles = async (response) => {
    ...
    if (multiple) {
      ...
      if (
        fileResponse?.length > maxFilesCount ||
        fileResponse?.length > maxFilesCount - fileList.length
      ) {
  ...
  return (
    <View>
      <TouchableOpacity
        ...
        style={[customComponent ? {} : styles.touchableOpacityStyle, style]}
        onPress={(event) => {
      ...
      {hintText && (
        <Text
          style={{
            color: AppColors.NEW_LIGHT_GRAY,
            fontSize: fontSizeConstants.RFValue12,
...
//             {
//                 Boolean(extraText) &&
//                 <Text style={styles.attachmentExtraText}>
//                     {extraText}
//                 </Text>
//             }
//         </View>
...
//     return (
//         <View style={[style]}>
//             {showTitle && <Text style={styles.headerText}>
