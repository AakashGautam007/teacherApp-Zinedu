import { TextProps, TouchableOpacity, TouchableOpacityProps, ViewProps, StyleSheet, View, FlatList, Image, Text, Platform } from 'react-native';
...
export const AttachmentButton = (props) => {
                ...
                async response => {
                    ...
                    if (response.didCancel) {
                        ...
                        if (await checkFiles(response?.assets)) {
                            ...
                            multiple ? setFileList([...fileList, ...response.assets]) : setFileList([...fileList, response.assets])
                        }
                        setLoading(false)
                    }
                })
    ...
    return <View>
        ...
        {hintText && <Text style={{ color: AppColors.NEW_LIGHT_GRAY, fontSize: fontSizeConstants.RFValue12, marginTop: 5 }}>* {hintText}</Text>}
    </View>
