...
const PastLiveClassInfo = ({navigation,route}) => {
    ...
    const getSchedule = async () =>{
        ...
        const response = await fetch(`${ENDPOINT}/student/get-teacher-live-class/?id=${item.liveclass_assoc.id}`,{
            headers:{
    ...
    const getStudioInfo = async () => {
        const response = await fetch(`https://sms.zinedu.com/get-studio-by-liveclass?id=${item.liveclass_assoc.id}`,{
            method:'GET',
            headers:{
                'Accept':'application/json',
                'Content-Type': 'application/json',
...
<ScrollView style={{flex:1}}>
        ...
        <TouchableOpacity 
            ...
            <TouchableOpacity 
            onPress={()=>navigation.navigate('ViewRecording',{link:item.live_class_url})}
            style={{width:'45%',justifyContent:'center',alignItems: 'center',backgroundColor:'#ECECEC',height:157,borderRadius:30}} >
...
<TouchableOpacity 
    onPress={()=>navigation.navigate('SubjectEdit',{item:liveClass,subj:liveClass.chapter_assoc.subject_assoc.subject_name})}
    style={{width:'45%',justifyContent:'center',alignItems: 'center',backgroundColor:'#ECECEC',height:157,borderRadius:30}} >
