...
const WannaAnsOrNot = ({ navigation, route }) => {
  ...
  const [items, setItems] = useState([
    ...
    {
      label: "Not comfortable in this topic",
      value: "not comfortable in this topic",
    },
  ...
  const response = async (id, ans, value) => {
    ...
    const response = await fetch(
      `${ENDPOINT}/support/update-alloted-doubt/${id}/`,
      {
        method: "PUT",
        headers: {
  ...
  const getDoubt = async () => {
    ...
    // const token = await AsyncStorage.getItem('userToken')
    const response = await fetch(
      `${ENDPOINT}/student/get-pending-doubts/?pending_doubt_id=${pending_doubt_assoc}`,
      {
        method: "GET",
        headers: {
          Accept: "application/json",
      ...
      <View style={styles.container}>
        ...
        >
          {question.student_query_assoc.query_file == null ? (
            <Text styles={{ fontFamily: typography.montserrat_400 }}>
              {question.student_query_assoc.query_text}
            </Text>
          ) : (
            <Image
              source={{ uri: question.student_query_assoc.query_file }}
