...
const NotesFile = ({ navigation, route }) => {
  ...
  const updateNotes = async () => {
    ...
    const response = await fetch(
      `${ENDPOINT}/support/update-teacher-notes/${notess[0].notes_assoc.id}/`,
      {
        body: formdata,
        method: "PUT",
        headers: {
  ...
  const addNotessss = async () => {
    ...
    const response = await fetch(
      `${ENDPOINT}/student/upload-live-class-notes/`,
      {
        method: "POST",
        headers: {
          Accept: "application/json",
  ...
  const deleteNotes = async () => {
    ...
    const response = await fetch(
      `${ENDPOINT}/support/delete-teacher-notes/${notess[0].notes_assoc.id}/`,
      request_params
    );
    const D = await response.json();
    setMessage(D);
  ...
  const onNotes = async () => {
    ...
    const response = await fetch(
      `${ENDPOINT}/student/get-live-class-notes/?live_class_id=${item.id}`,
      request_params
    );
    const D = await response.json();
    ...
    return (
      <View style={{ flex: 1, justifyContent: "center", alignItems: "center" }}>
        <Text style={{ fontFamily: typography.montserrat_400 }}>
          Notes uploading please wait Do not leave this page
        </Text>
        <ActivityIndicator size="large" color="black" />
      </View>
    );
      ...
      {notess.length == 0 ? (
        ...
        >
          ...
          <TextInput
            ...
            placeholder="Notes Decsription (compulsary)"
            style={styles.inp}
            value={txt}
            onChangeText={setTxt}
          />
          ...
          <TouchableOpacity
            onPress={() =>
              navigation.navigate("NotesView", {
                file: notess[0].notes_assoc.notes_file,
              })
            }
          >
            <Image
              source={{ uri: notess[0].notes_assoc.notes_icon }}
              style={{
                resizeMode: "contain",
                width: width * 0.85,
                height: height * 0.38,
