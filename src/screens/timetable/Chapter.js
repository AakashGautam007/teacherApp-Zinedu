...
const Chapter = ({navigation,route}) => {
    ...
    const response = await fetch(`${ENDPOINT}/support/get-chapters/?subject=${item.chapter_assoc.subject_assoc.id}`,requestOptions)
    const D = await response.json();
    console.log(D)
    setChapter(D)
...
<Button title="Update Chapter" onPress={()=>{
    return(
        chapterId!=''?updateChapter(chapterId):Alert.alert('Select a chapter First')
    )
}
    } />
        </View>
